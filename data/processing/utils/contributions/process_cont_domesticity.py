...
def divide_cands(cands_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def process_cont_domesticity(cands_df: pd.DataFrame, dists_df: pd.DataFrame, conts_df: pd.DataFrame):
    dist_cands_df, statewide_cands_df = divide_cands(cands_df)
