...
def process_other_contributions(year: str = None):
    print(f"\n{'-' * 100}\n{'-' * 100}\nStarted processing Other Contributions")
    file_type = "oth"
    if not year:
        year = decide_year()
    uri = get_mongo_uri()
    ...
    main_df = load_df_from_file(year, file_type, "itoth.txt", spark, headers, cols)
    ...
    print(f"\nFinished processing Other Contributions\n{'-' * 100}\n{'-' * 100}\n")
    spark.stop()


if __name__ == "__main__":
    process_other_contributions()
