...
def get_user_inputs(action: str, chamber: bool = None, data_dir = None):
    ...
    def decide_state(year: str, chamber: str = None):
        ...
        if not chamber:
            ...
            while True:
                ...
                if is_valid_input(choice = state_input, choices = states):
                    return state_input
                else:
                    print_retry_message(subject = subject)
                    continue
            ...
            while True:
                state_input = input_choice(subject = "state", action = action, choices = states).upper()
                if is_valid_input(choice = state_input, choices = states):
                    return state_input
                else:
                    print_retry_message(subject = subject)
    ...
    def decide_district(year: str, state: str, chamber: str = None):
        ...
        if not chamber:
            ...
            if len(districts) == 1:
                ...
                while True:
                    district_input = input_choice(subject = "district", action = action, choices = districts)
            ...
            district_input = input_choice(subject = "district", action = action)
            return district_input
    ...
    def decide_candidate(year: str, state: str, district: str):
        ...
        while True:
            candidate_input = input_choice(subject = "candidate", action = action, choices = candidate_last_names).upper()
    ...
    if chamber:
        ...
        chamber = None
    state = decide_state(chamber = chamber, year = year)
