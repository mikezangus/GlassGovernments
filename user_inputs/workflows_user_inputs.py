...
def determine_workflow_files(action: str, source: str, year: str, chamber: str, state: str, raw_data_dir: str):
    ...
    if chamber.lower() == "house":
        ...
        if state in us_state_at_large_list:
            ...
            district_list = output_district_list(action, source, year, chamber, state, raw_data_dir)
        if len(district_list) > 1:
            output_list_districts = process_multiple_districts([], source, year, chamber, state, district_list, raw_data_dir)
            return output_list_districts
        district = district_list[0]

        candidate_list = output_candidate_list(action, year, chamber, state, raw_data_dir, district)
        if len(candidate_list) > 1:
            output_list_candidates = process_multiple_candidates([], year, chamber, state, district, candidate_list, raw_data_dir)
        ...
        candidate_list = output_candidate_list(action, year, chamber, state, raw_data_dir)
        if len(candidate_list) > 1:
            output_list_candidates = process_multiple_candidates([], year, chamber, state, candidate_list = candidate_list, raw_data_dir = raw_data_dir)
            return output_list_candidates
        candidate = candidate_list[0]
...
def determine_workflow(action: str, source: str, raw_data_dir: str):
     
    year_list = output_year_list(action, source, raw_data_dir)
    if len(year_list) > 1:
        output_list = process_multiple_years([], source, year_list, raw_data_dir)
        return output_list
    year = year_list[0]

    chamber_list = output_chamber_list(action)
    if len(chamber_list) > 1:
        output_list = process_multiple_chambers([], source, year, chamber_list, raw_data_dir)
        return output_list
    chamber = chamber_list[0]

    state_list = output_state_list(action, source, year, chamber, raw_data_dir)
    if len(state_list) > 1:
        output_list = process_multiple_states([], source, year, chamber, state_list, raw_data_dir)
        return output_list
    state = state_list[0]
    ...
    if source.lower() == "internet":
        ...
        output_list = determine_workflow_files(action, source, year, chamber, state, raw_data_dir)
    return output_list