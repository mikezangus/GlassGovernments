...
def download_app(dir: str, url: str, file_name: str) -> bool:
    try:
        print(
...
def download_app() -> bool:
    url, file_extension = determine_os()
    file_name = os.path.join(CURRENT_DIR, f"firefox_latest{file_extension}")
    file_path = os.path.join(os.path.abspath(file_name))
    if not download_app(CURRENT_DIR, url, file_name):
        return False
