...
from modules.utilities import load_web_page, handle_rate_limit
    ...
    def process_candidate_info(elements):
        ...
        try:
            ...
            if total_receipts == 0.00 or 0:
                print(f"{state}-{district} candidate {candidate}/{candidate_count}: {first_name} {last_name} received ${total_receipts:,.2f} in donations, moving on")
            ...
            time.sleep(1)
            print(f"{state}-{district} candidate {candidate}/{candidate_count}: Starting to scrape {first_name} {last_name} ({party}), who received ${total_receipts:,.2f} in donations")
    ...
    def check_if_export_available(first_name, last_name):
        ...
        for attempt in range(max_attempts + 1):
            ...
            if "is-disabled" not in element_export_button.get_attribute("class"):
                ...
                if "this table has no data to export" in export_message.text.lower():
                    message = f"No data to export for {subject}, skipping"
                    print(message)
                    logging.info(message)
                    return False
                elif donation_count >= 500000 and "bulk data" in export_message.text.lower():
                    message = f"{state}-{district} candidate {first_name} {last_name}'s data needs to be downloaded via bulk data at https://www.fec.gov/data/browse-data/?tab=bulk-data"
                    print(message)
                    logging.info(message)
                    return False
                elif "is-disabled" in element_export_button.get_attribute("class"):
        ...
        def process_download_sequence():
            ...
            if not click_download_button(driver = driver, locator = locator_download_button, state = state, district = district, first_name = first_name, last_name = last_name):
                if not handle_rate_limit(driver = driver, locator = locator_downloads_pane, state = state, district = district, first_name = first_name, last_name = last_name):
                    return False
            if not save_downloaded_file(year = year, chamber = chamber.capitalize(), state = state, district = district, last_name = last_name, first_name = first_name, party = party):
                return False
